...
/// Parameters:
///   path: string (required) — supports {ts}, {ts_ms} template vars
///   data: optional override for what to dump (defaults to current data)
#[async_trait]
impl StepHandler for DumpStep {
    fn name(&self) -> &'static str {
        "dump"
    }
    ...
    async fn execute(
        ...
        fs::write(path, &content)
            .map_err(|e| CliError::pipeline(format!("dump: failed to write file: {e}")))?;

        info!(path = resolved_path, "Dumped pipeline data to file");
...
pub fn register_transform_steps(registry: &mut StepRegistry) {
    ...
    registry.register(Arc::new(DumpStep));
...
mod tests {
    ...
    #[tokio::test]
    async fn test_dump_step() {
        use std::fs;

        let tmp_dir = std::env::temp_dir();
        let file_path = tmp_dir.join("opencli_test_dump_{ts}.json");
