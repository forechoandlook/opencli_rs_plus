...
impl StepHandler for DownloadStep {
    ...
    async fn execute(
        ...
        if download_type == "base64-batch" {
            return execute_base64_batch_download(obj.unwrap_or(&serde_json::Map::new()), &ctx, data).await;
        }
...
async fn execute_base64_batch_download(
    ...
    if items.is_empty() {
        ...
        let debug_msg = format!("No images to download. Data structure: {}",
    ...
    if results.is_empty() {
        result.insert("download_results".to_string(), serde_json::json!([{ "filename": "-", "status": "no images", "size": "-" }]));
    } else {
