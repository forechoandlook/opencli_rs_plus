...
抓取失败时，按下面的顺序排查，能跳过 daemon/scheduler 就跳过，链路越短越好定位问题。

## 第 0 层：先分清是哪条链路

```
opencli <site> <cmd>              # 直接执行 adapter，不经过调度 daemon
    │
...
大多数"抓不到数据"的问题出在 browser:true 这条线上（插件没连上/页面选择器错/接口变了），不是调度 daemon 的问题。**先确认你的 adapter 是不是 `browser: true`，不是的话直接跳到第 3 层。**
...
- 不需要启动 `opencli daemon`，也不需要 job/scheduler 介入
...
## 第 4 层：只有涉及 job/scheduler/多次调度时才需要

如果问题只在"定时任务不触发"或"重复执行行为异常"时出现，才需要真正启动调度 daemon：

```bash
OPENCLI_VERBOSE=1 nohup cargo run -- daemon > /tmp/opencli-daemon.log 2>&1 &
cargo run -- job add "<site> <cmd>" --delay 5
tail -f /tmp/opencli-daemon.log
```

这是最长的链路（daemon → browser-daemon → 插件 → CDP），只在前三层都排除后才进入。日常开发新 adapter 完全不需要走到这一层。
...
| 定时任务没跑 / 跑重了 | 第4层，daemon 日志 + `opencli job show <id>` |
